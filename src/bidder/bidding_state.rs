...
impl Opening {
  pub fn new(dir: Dir, vuln: Vulnerability, hand: Hand) -> Opening {
    ...
    Opening { dir, hand, vuln, hand_eval }
  }
}

impl BiddingState for Opening {
  fn observe_bid(&mut self, dir: &Dir, bid: &Bid) {
    if self.dir == *dir { panic!("Cannot observe own bid!") }
    match bid {
      Bid::Pass => (),
  ...
  fn next_bid(&self) -> ProposedBids {
    ...
    if total_points > GF_OPENING_THRESHOLD {
      response.insert(Bid::Call(Call { level: 2, trump: Some(Suit::Club) }), 1);
    }
    if shape == ShapeType::Balanced {
      if ONE_NO_TRUMP_RANGE.contains(&total_points) {
        response.insert(Bid::Call(Call { level: 1, trump: None }), 10);
      } else if TWO_NO_TRUMP_RANGE.contains(&total_points) {
        response.insert(Bid::Call(Call { level: 2, trump: None }), 10);
      }
    }
    match self.hand_eval.shape {
      Shape([_, _, _, 5..=13]) => // 5+ Spades
        response.insert(Bid::Call(Call { level: 1, trump: Some(Suit::Spade) }), 5),
      Shape([_, _, 5..=13, _]) => // 5+ Hearts
        response.insert(Bid::Call(Call { level: 1, trump: Some(Suit::Heart) }), 5),
      Shape([_, 4..=13, _, _]) => // 4+ Diamonds
        response.insert(Bid::Call(Call { level: 1, trump: Some(Suit::Diamond) }), 5),
      _ =>
        response.insert(Bid::Call(Call { level: 1, trump: Some(Suit::Club) }), 5)
    };
